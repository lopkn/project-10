...
function wglInit() {
    ...
    wglCan.render = (t = 2) => {
        ...
        if (circles.update) {
            gl.bindBuffer(gl.ARRAY_BUFFER, circles.buffer);
            gl.bufferSubData(gl.ARRAY_BUFFER, 0, circles.vertexData);
...
function swept_ball_to_line_collision(...inarr) { // assume dt = 1
    ...
    // Check if distance from endpoints of line segment to the capsule's path is less than r (for cases where line is short and ball passes over it)
    ...
    if (ball_end_to_line.dist < r ) { // the ball's end point overlaps the wall, and the ball is not approaching from sides, and its only the end cap
      ...
      if(!collision || t <= collision.t){
        ...
        // Check if the distance from the line segment to either endpoint of the capsule is less than r
