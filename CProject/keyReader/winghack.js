...
window.main = 
  ...
  if(window.B[pla] == undefined||window.B[pla].inGame == false){
    ...
    if(B[pla] && last[pla]){
      let PLAYT2 = tran(B[pla].dstX,B[pla].dstY)
      if(PLAYT2[0] != last[pla].lx || PLAYT2[1] != last[pla].ly){
        FRAME()
      }
    }
    
  ctx.fillText(Math.floor(COUNTER2%16),Width/2,Height/2)
...
objk.forEach((a,i)=>{
  let e = window.B[a]
    ...
    if(last[a] == undefined){
      last[a] = {"lx":t[0],"ly":t[1],"lvy":0,"lvx":0,"vx":e.x-e.prevX,"vy":e.y-e.prevY,"ax":0,"ay":0,"turn":0}
      last2[a] = {"lx":t[0],"ly":t[1],"lvy":0,"lvx":0,"vx":e.x-e.prevX,"vy":e.y-e.prevY,"ax":0,"ay":0}
    }
     // else {
     ...
     //  }
      ...
      let t3 = tran(e.dstX,e.dstY)
      let t2 = [e.dstX,e.dstY]
      if(t2[0] != last[a].lx || t2[1] != last[a].ly){
        last[a] = {"turn":e.dstAngle-e.origAngle,"lx":t2[0],"ly":t2[1],"lvx":t2[0]-last[a].lx,"lvy":t2[1]-last[a].ly,"vx":t2[0]-last[a].lx,"vy":t2[1]-last[a].ly,"ax":t2[0]-last[a].lx-last[a].vx,"ay":t2[1]-last[a].ly-last[a].vy}
        last2[a] = {"lx":t3[0],"ly":t3[1],"lvx":t3[0]-last2[a].lx,"lvy":t3[1]-last2[a].ly,"vx":t3[0]-last2[a].lx,"vy":t3[1]-last2[a].ly,"ax":t3[0]-last2[a].lx-last2[a].vx,"ay":t3[1]-last2[a].ly-last2[a].vy}
      }
    }
      // last[a] = {}
  ...
  if(closest !== -1 && toggleDraw){
    ...
    if(lock){
      ...
      ctx.lineTo(last2[closest].vx*adjust  + t[0]-lx,last2[closest].vy*adjust +t[1]-ly)
      ctx.stroke()
      ctx.beginPath()
      ctx.strokeStyle = "#FF0000"
      ctx.moveTo(last2[closest].vx*adjust  + t[0]-lx,last2[closest].vy*adjust +t[1]-ly)
      ctx.lineTo(last2[closest].vx*adjust+last2[closest].ax*adjust  + t[0]-lx,last2[closest].vy*adjust+last2[closest].ay*adjust +t[1]-ly)
      ctx.stroke()
...
function autoF(c){
  ...
  // console.log(tp[0])
      ...
      // -(window.B[c].y+last[c].vy*Tlead+last[c].ay*TTlead-window.B[pla].y+bot.aimAddy-bot.permAimY)
        ...
        // let frameMarchShotpos = [
        //   window.B[c].x+deltaV[0]*lead+0-window.B[pla].x+bot.aimAddx,
        // -(window.B[c].y+deltaV[1]*lead+0-window.B[pla].y+bot.aimAddy-bot.permAimY)
        // ]
  ...
  if(downs["x"] == true || bot.on){
    ...
    if(Adist > botHandycap){
      window.A?.sendShooting(0)
      return;
    }


    let AshootThreshold = Math.atan(shootThreshold*150/Adist)

    if(downs["z"]){
      if(closestAng < AshootThreshold && (B[pla].weapon != 8 || B[c].dashing() == false)){
        window.A?.sendShooting(1)
      }
    } else if( ((Math.abs(aGame-aEnemy) < AshootThreshold && distP(pla,closest) < minDist)/*|| closestAng < shootThreshold */)&&(bot.on||downs["x"])){
    if(closestFriendly > friendlyThreshold ){
      window.A?.sendShooting(1)
    } else if(Math.abs(aGame-aEnemy) > 2*AshootThreshold){
      console.log("stopped")
      window.A?.sendShooting(0)
    }
  } else if(Math.abs(aGame-aEnemy) >AshootThreshold*2 && !mouseDown){
    window.A?.sendShooting(0)
  }
  if(closestFriendly < friendlyThreshold){
    window.A?.sendShooting(0)
...
function UangToPSerialised(ang){
