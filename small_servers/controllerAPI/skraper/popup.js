...
function s(str,t){chrome.tabs.query({active: true}, (tabs) => {
    if (tabs[0]) {
        if(t==1){
            chrome.tabs.sendMessage(tabs[0].id, {type: 'eval', msg: str});
        } else {
            chrome.tabs.sendMessage(tabs[0].id, {type: 'msg', msg: str});
        }
    }
});}
